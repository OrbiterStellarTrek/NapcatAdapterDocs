...
请确保：
1. 提问时请带上报错日志和报错聊天截图
2. 请确保涉及插件/Napcat-Adapter已是最新“#nc更新”“#xx插件更新”
...
而 TRSS-Yunzai 对此机制进行了改进：它在生成唯一标识时，额外加入了当前 Bot 自身的 ID（例如 "789456123:123456789:#帮助"）。由于不同 Bot 的 ID 不同，即使消息内容 (raw_msg) 和发送者 QQ 号相同，生成的标识也总是唯一的。因此，TRSS-Yunzai 能有效支持多个 Bot 在同一个群内同时响应消息。
